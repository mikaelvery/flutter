...
import 'package:pizzaapp/widget/delivery_tracking.dart';
import 'package:provider/provider.dart';

LatLng userDeliveryLocation = const LatLng(43.656693, 3.935781); // Coordonnées de livraison 

class CartPage extends StatefulWidget {
  final List<CartItem> cartItems; // List de mon panier

  const CartPage({super.key, required this.cartItems}); // Construct prend une list D'items
...
class CartPageState extends State<CartPage> {
  ...
  Widget build(BuildContext context) {
    final userProvider = Provider.of<UserProvider>(context); // Récupération de Userprovider via le provider
  ...
  void addOrderForUser(String userId, double amount, List<CartItem> cartItems) async {
    try {
    ...
    addOrderForPizzeria(amount, cartItems, 'Ln5g94LnWG0zvvUfNhc6', orderId, userId); 
...
void addOrderForPizzeria(double amount, List<CartItem> cartItems, String pizzeriaId, String orderId, String userId) async {
  try {
    ...
    if (userSnapshot.exists) {
      ...
      if (orderSnapshot.exists) {
        ...
        await FirebaseFirestore.instance.collection('pizzeria').doc(pizzeriaId).collection('orders').doc(orderId).set({
          'amount': amount,
          'userId': userId,
        ...
        if (kDebugMode) {
          print('ID de la commande transmise à la pizzeria pour l\'utilisateur $userId avec le numéro $orderId');
        }
      } else {
        if (kDebugMode) {
          print('La commande avec l\'ID $orderId n\'existe pas dans la collection "orders".');
        }
      }
    } else {
      if (kDebugMode) {
        print('L\'tilisateur avec l\'ID $userId n\'existe pas dans la collection "users".');
      }
    }
    } catch (e) {
      if (kDebugMode) {
        print('Désolé la commande n\'a pas pu être transmise à la pizzeria ! : $e');
      }
    }
  }
}